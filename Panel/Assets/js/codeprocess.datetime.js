...
function ToLatinDatetime(shamsiInput) {
    var temp = shamsiInput.split("/");
    var year = parseInt(temp[0]);
    var month = parseInt(temp[1]);
    var day = parseInt(temp[2]);

    var grgDate = ToGregorian(year, month, day);

    return grgDate;
}

function ToShamsiDatetimeWithTimeSpan(grgDateInput, Format) {
    if (grgDateInput != null) {
        var grgDate = new Date(grgDateInput.toString());
        var temp = ToShamsi(grgDate.getFullYear(), grgDate.getMonth() + 1, grgDate.getDate(), Format);
        temp = temp.replace(" ", "").replace(" ", "");
        return temp + "&nbsp;&nbsp;&nbsp;" + grgDate.getHours() + ":" + grgDate.getMinutes();
    } else {
        return "";
    }
}

function ToShamsiDatetime(grgDateInput, Format) {
    if (grgDateInput != null) {
        var grgDate = new Date(grgDateInput.toString());
        var temp = ToShamsi(grgDate.getFullYear(), grgDate.getMonth() + 1, grgDate.getDate(), Format);
        temp = temp.replace(" ", "").replace(" ", "");
...
function ToShamsiTimeSpan(grgDateInput) {
    if (grgDateInput != null) {
        var grgDate = new Date(grgDateInput.toString());
        return grgDate.getHours() + ":" + grgDate.getMinutes();
    } else {
        return "";
    }
}

function ToSimpleTime(grgTimeSpan) {
    var temp = grgTimeSpan.toString().split(":");
    return temp[0] + ":" + temp[1];
}

String.prototype.toTimeSpanFormat = function () {
