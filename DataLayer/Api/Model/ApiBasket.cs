...
namespace DataLayer.Api.Model
{
    public class ApiBasket : ApiResponse
    {
        ...
        public static ApiResult Post(UnitOfWork _context, int? accountId, int productId, int? colorId = null, int? sizeId = null, int? optionItemId = null, int? resellerId = null, int? storeId = null, bool exteraResult = false,int count=1, AccountAddress address = null)
        {
            ...
            {
                if(BaseWebsite.WebsiteSetting.HasMinOrder == true)
                {
                    return CreateErrorResult(Enum_Message.INVALID_PRODUCT_QUANTITY_COLOR_MinOrder);
                }
        ...
        public static ApiResult Put(UnitOfWork _context, string actionType, int? accountId = null, int? basketId = null, int? productId = null, int? colorId = null, int? sizeId = null, int? optionItemId = null, int? storeId = null, bool exteraResult = false, AccountAddress address=null)
        {
            ...
            if (basket.StoreId == null)
            {
                ...
                if (quantity != null)
                {
                    ...
                    if (Base.BaseWebsite.WebsiteSetting.HasMinOrder && quantity.Product.MinOrder.HasValue)
                    {
                        if ((actionType == "add" ) && (basket.Count + 1) < quantity.Product.MinOrder.Value)
                        {
                            return CreateErrorResult(Enum_Message.INVALID_PRODUCT_QUANTITY_COLOR_MinOrder);

                        }
                        if ((actionType == "remove") && (basket.Count-1 ) < quantity.Product.MinOrder.Value)
                        {
                            return CreateErrorResult(Enum_Message.INVALID_PRODUCT_QUANTITY_COLOR_MinOrder);
            ...
            else
            {
                ...
                if (storeProductQuantity != null)
                {
                    ...
                    if (Base.BaseWebsite.WebsiteSetting.HasMinOrder && storeProductQuantity.StoreProduct.Product.MinOrder.HasValue)
                    {
                        if ((actionType == "add" ) && (basket.Count + 1) < storeProductQuantity.StoreProduct.Product.MinOrder.Value)
                        {
                            return CreateErrorResult(Enum_Message.INVALID_PRODUCT_QUANTITY_COLOR_MinOrder);

                        }
                        if ( actionType == "remove" && (basket.Count-1) < storeProductQuantity.StoreProduct.Product.MinOrder.Value)
                        {
                            return CreateErrorResult(Enum_Message.INVALID_PRODUCT_QUANTITY_COLOR_MinOrder);
