...
namespace DataLayer.Repositories
{
    public class Entity_Payment : BaseRepository<Payment>
    {
        ...
        public void DoPaymentServices(UnitOfWork mainContext, Payment payment)
        {
            ...
            if (BaseWebsite.WebsiteLabel.Contains("hvachome") || BaseWebsite.WebsiteLabel.Contains("HavacHome"))
            {
                bool haveCourse = payment.PaymentProductOrder.Any(s => s.AccountOrder.AccountOrder1.Any(x => x.AccountOrderProduct.Any(p=>p.Product.ProductTypeId == 2)));
                if (haveCourse == true)
                {
                    var client = new RestClient("https://dl.hvachome.ir/wp-json/jwt-auth/v1/token");
                    client.Timeout = -1;
                    var request = new RestRequest(Method.POST);
                    request.AddParameter("username", "info@hvachome.ir");
                    request.AddParameter("password", "dnskj3453#$^ERfrg");
                    IRestResponse response = client.Execute(request);
                    LavinApi.GetToken tokenClass = JsonConvert.DeserializeObject<LavinApi.GetToken>(response.Content);

                    var client2 = new RestClient("https://dl.hvachome.ir/wp-json/player/v1/getuser");
                    client2.Timeout = -1;
                    var request2 = new RestRequest(Method.POST);
                    request2.AddParameter("phone_number", payment.Account.Mobile);
                    request2.AddHeader("Authorization", "Bearer " + tokenClass.token);
                    IRestResponse response2 = client2.Execute(request2);
                    LavinApi.GetUser User = JsonConvert.DeserializeObject<LavinApi.GetUser>(response2.Content);
                    if (User.statusCode != 200)
                    {
                       
                        var client3 = new RestClient("https://dl.hvachome.ir/wp-json/player/v1/createuser");
                        client3.Timeout = -1;
                        var request3 = new RestRequest(Method.POST);
                        request3.AddHeader("Authorization", "Bearer " + tokenClass.token);
                       
                        //request3.AddHeader("Token", tokenClass.token);
                        request3.AddParameter("username", payment.Account.Mobile);
                        request3.AddParameter("password", payment.Account.VerifyToken);
                        request3.AddParameter("email", payment.Account.Email);
                        request3.AddParameter("role", "subscriber");
                        request3.AddParameter("first_name", payment.Account.FullName);
                        request3.AddParameter("last_name", payment.Account.FullName);
                        request3.AddParameter("display_name", "wordpress test user");
                        request3.AddParameter("phone_number", payment.Account.Mobile);
                        IRestResponse response3 = client3.Execute(request3);
                        LavinApi.CreateUser CreateUser = JsonConvert.DeserializeObject<LavinApi.CreateUser>(response3.Content);
                        foreach (var item in payment.PaymentProductOrder)
                        {
                            foreach (var productOrder in item.AccountOrder.AccountOrder1)
                            {
                                foreach(var productItem in productOrder.AccountOrderProduct)
                                {
                                    if (productItem.Product.ProductTypeId == 2)
                                    {
                                        var client4 = new RestClient("https://dl.hvachome.ir/wp-json/player/v1/createlicense");
                                        client4.Timeout = -1;
                                        var request4 = new RestRequest(Method.POST);
                                        request4.AddParameter("user_id", CreateUser.user_id);
                                        request4.AddHeader("Authorization", "Bearer " + tokenClass.token);
                                        request4.AddParameter("product_name", productItem.Product.Name);
                                        request4.AddParameter("product_sku", productItem.Product.ID);
                                        request4.AddParameter("product_pass", productItem.Product.CodeValue);
                                        request4.AddParameter("limit_use_windows", "1");
                                        request4.AddParameter("order_price", productItem.Product.Price);
                                        request4.AddParameter("enable_license_fast_disable", "on");
                                        request4.AddParameter("license_status", "1");
                                        request4.AddParameter("order_desc", "");
                                        request4.AddParameter("post_status", "publish");
                                        IRestResponse response4 = client4.Execute(request4);
                                        LavinApi.CreateLinces crateLinces = JsonConvert.DeserializeObject<LavinApi.CreateLinces>(response4.Content);
                                    }
                                }
                            ...
                            foreach (var productOrder in item.AccountOrder.AccountOrder1)
                            {
                                foreach (var productItem in productOrder.AccountOrderProduct)
                                {
                                    if (productItem.Product.ProductTypeId == 2)
                                    {
                                        ...
                                        var request4 = new RestRequest(Method.POST);
                                        request4.AddHeader("Authorization", "Bearer " + tokenClass.token);
                                        request4.AddParameter("user_id", User.ID);
                                        request4.AddParameter("product_name", productItem.Product.Name);
                                        request4.AddParameter("product_sku", productItem.Product.ID);
                                        request4.AddParameter("product_pass", productItem.Product.CodeValue);
                                        request4.AddParameter("limit_use_windows", "1");
